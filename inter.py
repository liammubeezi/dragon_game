...
def new():
    ...
    while run:
        ...
        for event in pygame.event.get(): 
            ...
            if trigger < int(50) and len(target_spawn) < 7:
        ...
        elif dragon.x >= WIDTH - 21 - DRAG_HEI:
            collide_sound.play()
            draw_text(boundary, text_font, RED, 220, 150)
            pygame.display.update()
            pygame.time.delay(2000)
        ...
        elif dragon.y >= HEIGHT - 5 - DRAG_HEI:
            collide_sound.play()
            draw_text(boundary, text_font, RED, 220, 150)
            pygame.display.update()
            pygame.time.delay(2000) 
...
def handle_target(target_spawn, dragon):
    for target in target_spawn[:]:  
        distance = ((target.x - dragon.x) ** 2 + (target.y - dragon.y) ** 2) ** 0.5
        
        if distance <= 200: 
            if target.x < dragon.x and target.x - TARGET_VEL > 0: 
                target.x -= TARGET_VEL
            elif target.x > dragon.x and target.x + TARGET_WID + TARGET_VEL < WIDTH:  
                target.x += TARGET_VEL
            if target.y < dragon.y and target.y - TARGET_VEL > 0:  
                target.y -= TARGET_VEL
            elif target.y > dragon.y and target.y + TARGET_HEI + TARGET_VEL < HEIGHT: 
                target.y += TARGET_VEL
